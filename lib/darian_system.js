// MARS_NORTH_SPRING_SOLSTICE = '06 March 1878';
'use strict';
...
function convMarsToEarth(mDate) {
  ...
  var year = parseInt(mDate['mYear'], 10);
  var month = parseInt(mDate['mMonth'], 10);
  var day  = parseInt(mDate['mDay'], 10);
  var TestData = 3;
  ...
  var hour = parseInt(mDate['mHour'], 10)/24
  var min  = parseInt(mDate['mMin'], 10)/1440
  var sec  = parseInt(mDate['mSec'], 10)/86400;
...
class Darian_Date {
  constructor() {
    ...
    if (arguments.length >= 1 && typeof arguments[0] != 'object') {
      ...
      if (arguments[6]) {
        timeZoneCorrector = parseInt(arguments[6], 10);
      } else {
        timeZoneCorrector = 0;
      }

      eYear = parseInt(arguments[0], 10);
      eMonth = parseInt(arguments[1], 10) || 1;
      eDay = parseInt(arguments[2], 10) || 1;
      ...
      eHour = (parseInt(arguments[3], 10)-timeZoneCorrector)/24 || 0;
      eMin = parseInt(arguments[4], 10)/1440 || 0;
      eSec = parseInt(arguments[5], 10)/86400 || 0;

      if (!isNaN(eHour)) daysSince += eHour;
      if (!isNaN(eMin))  daysSince += eMin;
      if (!isNaN(eSec))  daysSince += eSec;
    }
    ...
    if (Math.round(ls*10)/10 < 10) {
      this.mSolarLongitude = "00" + Math.round(ls*10)/10;
    } else {
      if (Math.round(ls*10)/100 < 10) {
        this.mSolarLongitude = "0" + Math.round(ls*10)/10;
      } else {
        this.mSolarLongitude = Math.round(ls*10)/10;
      }
    }
  ...
  getDate() {
    var output = '';
    if (this.mSolName != undefined) {
      output += this.mSolName+', ';
      if (this.mDay != undefined) {
        output += this.mDay+' ';
        if (this.mMonthName != undefined) {
          output += this.mMonthName+' ';
          if (this.mYear != undefined) {
            output += this.mYear+', ';
            if (this.mHour != undefined) {
              output += this.mHour+':';
              if (this.mMin != undefined) {
                output += this.mMin+':';
  ...
  static threeDigit(n) {
  	if (n < 0)
  		return "-" + Darian_Date.PadLeft(-n, 3, "0");
  	else
  		return Darian_Date.PadLeft(n, 3, "0");
  }
  static fiveDigit(n) {
  	if (n < 0)
  		return "-" + Darian_Date.PadLeft(-n, 5, "0");
  	else
  		return Darian_Date.PadLeft(n, 5, "0");
  }
